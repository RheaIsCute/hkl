...
static void ForceCloseSandboxProcess(bool autoRelaunch) {
    ...
    if (autoRelaunch) {
        ...
        // Fallback: Check standard client path (obfuscated)
        wchar_t clientPathArr[] = { 0x19 ^ 0x5A, 0x60 ^ 0x5A, 0x06 ^ 0x5A, 0x08 ^ 0x5A, 0x33 ^ 0x5A, 0x35 ^ 0x5A, 0x2E ^ 0x5A, 0x7A ^ 0x5A, 0x1D ^ 0x5A, 0x3B ^ 0x5A, 0x37 ^ 0x5A, 0x3F ^ 0x5A, 0x29 ^ 0x5A, 0x06 ^ 0x5A, 0x08 ^ 0x5A, 0x33 ^ 0x5A, 0x35 ^ 0x5A, 0x2E ^ 0x5A, 0x7A ^ 0x5A, 0x19 ^ 0x36 ^ 0x5B, 0x33 ^ 0x5A, 0x3F ^ 0x5A, 0x34 ^ 0x5A, 0x2E ^ 0x5A, 0x06 ^ 0x5A, 0x08 ^ 0x5A, 0x33 ^ 0x5A, 0x35 ^ 0x5A, 0x2E ^ 0x5A, 0x19 ^ 0x36 ^ 0x5B, 0x33 ^ 0x5A, 0x3F ^ 0x5A, 0x34 ^ 0x5A, 0x2E ^ 0x5A, 0x09 ^ 0x5A, 0x3F ^ 0x5A, 0x28 ^ 0x5A, 0x2C ^ 0x5A, 0x33 ^ 0x5A, 0x39 ^ 0x5A, 0x3F ^ 0x5A, 0x29 ^ 0x5A, 0x74 ^ 0x5A, 0x3F ^ 0x5A, 0x22 ^ 0x5A, 0x3F ^ 0x5A, 0 };
        wchar_t launchArgsArr[] = { 0x77 ^ 0x5A, 0x77 ^ 0x5A, 0x36 ^ 0x5A, 0x3B ^ 0x5A, 0x2F ^ 0x5A, 0x34 ^ 0x5A, 0x39 ^ 0x5A, 0x32 ^ 0x5A, 0x77 ^ 0x5A, 0x2A ^ 0x5A, 0x28 ^ 0x5A, 0x35 ^ 0x5A, 0x3E ^ 0x5A, 0x2F ^ 0x5A, 0x39 ^ 0x5A, 0x2E ^ 0x5A, 0x67 ^ 0x5A, 0x2C ^ 0x5A, 0x3B ^ 0x5A, 0x36 ^ 0x5A, 0x35 ^ 0x5A, 0x28 ^ 0x5A, 0x3B ^ 0x5A, 0x34 ^ 0x5A, 0x2E ^ 0x5A, 0x7A ^ 0x5A, 0x77 ^ 0x5A, 0x77 ^ 0x5A, 0x36 ^ 0x5A, 0x3B ^ 0x5A, 0x2F ^ 0x5A, 0x34 ^ 0x5A, 0x39 ^ 0x5A, 0x32 ^ 0x5A, 0x77 ^ 0x5A, 0x2A ^ 0x5A, 0x3B ^ 0x5A, 0x2E ^ 0x5A, 0x39 ^ 0x5A, 0x32 ^ 0x5A, 0x36 ^ 0x5A, 0x33 ^ 0x5A, 0x34 ^ 0x5A, 0x3F ^ 0x5A, 0x67 ^ 0x5A, 0x36 ^ 0x5A, 0x33 ^ 0x5A, 0x2C ^ 0x5A, 0x3F ^ 0x5A, 0 };
        
        std::wstring riotClientPath(clientPathArr);
        std::wstring launchArgs(launchArgsArr);
        
        SecureZeroMemory(clientPathArr, sizeof(clientPathArr));
        SecureZeroMemory(launchArgsArr, sizeof(launchArgsArr));

        if (GetFileAttributesW(riotClientPath.c_str()) != INVALID_FILE_ATTRIBUTES) {
            std::cout << "[+] Launching via client..." << std::endl;
            ShellExecuteW(NULL, L"open", riotClientPath.c_str(), launchArgs.c_str(), NULL, SW_SHOWNORMAL);
            std::cout << "[+] Launch command sent!" << std::endl;
        } else {
            std::cout << "[-] Client path not found. Launch manually." << std::endl;
        }

        SecureWipeString(riotClientPath);
        SecureWipeString(launchArgs);
    } else {
