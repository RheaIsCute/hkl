...
std::wstring GetExportedFunctionName(const std::wstring& dllPath) {
    ...
    if (StealthAPI::pLoadLibraryExW) {
        hModule = StealthAPI::pLoadLibraryExW(dllPath.c_str(), NULL, DONT_RESOLVE_DLL_REFERENCES);
    } else {
        hModule = LoadLibraryExW(dllPath.c_str(), NULL, DONT_RESOLVE_DLL_REFERENCES);
    }
    if (!hModule) return L"";
    ...
    if (!exportDir) {
        if (StealthAPI::pFreeLibrary) StealthAPI::pFreeLibrary(hModule);
    ...
    for (DWORD i = 0; i < exportDir->NumberOfNames; i++) {
        ...
        if (strcmp(functionName, "DllMain") != 0) {
            ...
            result = wName;
            // Wipe the function name we just read
            break;
        }
    }
